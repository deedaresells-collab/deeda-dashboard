"""Market regime classification — no look-ahead."""
...
def _rolling_atr(price: pd.Series, window: int = 14) -> pd.Series:
    """ATR proxy from close-to-close absolute returns (no future data)."""
    tr = price.diff().abs()
    return tr.rolling(window, min_periods=max(2, window // 3)).mean()


def compute_regime_features(crypto_df: pd.DataFrame, ma_window: int = 20) -> pd.DataFrame:
    """
    Compute regime features per asset on crypto snapshots only.
    All rolling stats use past data exclusively.
    """
    frames = []
    for asset, grp in crypto_df.sort_values("timestamp").groupby("asset"):
        ...
        g["ma_20"] = price.rolling(ma_window, min_periods=max(3, ma_window // 4)).mean()
        g["dist_from_ma"] = (price - g["ma_20"]) / g["ma_20"].replace(0, np.nan)
        roll_std = ret.rolling(ma_window, min_periods=max(3, ma_window // 4)).std().replace(0, np.nan)
        g["return_zscore"] = ret / roll_std
        g["realized_vol_regime"] = g.get(
            "realized_vol_1h", ret.rolling(60, min_periods=10).std() * np.sqrt(60)
        ).fillna(0.5)
        g["atr"] = _rolling_atr(price, window=14)
        g["atr_pct"] = g["atr"] / price.replace(0, np.nan)

        vol_col = "volume_24h" if "volume_24h" in g.columns else None
        if vol_col:
            vol_ma = g[vol_col].rolling(ma_window, min_periods=3).mean().replace(0, np.nan)
        ...
        roll_high = price.rolling(ma_window, min_periods=3).max().shift(1)
        roll_low = price.rolling(ma_window, min_periods=3).min().shift(1)
        g["breakout_high"] = (price > roll_high).astype(float)
        g["breakout_low"] = (price < roll_low).astype(float)

        sign = np.sign(ret)
        persistence = sign.rolling(10, min_periods=3).apply(lambda x: abs(x.sum()) / len(x), raw=True)
...
def classify_regime_row(row: pd.Series) -> tuple[Regime, float]:
    """
    Rule-based regime classifier returning (regime, confidence).
    Transparent scoring — no ML.
    """
    mr_score = 0.0
    mom_score = 0.0
    ...
    # Mean reversion: extreme z-score, far from MA, low persistence
    if z > 1.5:
        mr_score += 0.3
    if z > 2.0:
        mr_score += 0.2
    ...
    # Momentum: breakouts, persistence, volume, OB alignment
    ...
    if mr_score > mom_score and mr_score >= 0.45:
        return Regime.MEAN_REVERTING, mr_score
    if mom_score > mr_score and mom_score >= 0.45:
        return Regime.MOMENTUM_TRENDING, mom_score
    return Regime.UNCERTAIN, max_score


def classify_regimes(crypto_df: pd.DataFrame) -> pd.DataFrame:
    """Classify regime for every crypto snapshot."""
    featured = compute_regime_features(crypto_df)
    regimes = []
    confidences = []
    for row in featured.itertuples(index=False):
        r, c = classify_regime_row(row._asdict())
...
REGIME_COLUMNS = [
    ...
    "uncertainty_buffer", "prob_change",
]
...
def merge_regime_to_snapshots(df: pd.DataFrame, regime_df: pd.DataFrame) -> pd.DataFrame:
    ...
    regime_cols = [
        ...
        "directional_persistence", "short_return",
    ]
    ...
    for asset, df_asset in base.groupby("asset", sort=False):
        ...
        merged = pd.merge_asof(da, r_asset, on="timestamp", direction="backward")
        parts.append(merged)
